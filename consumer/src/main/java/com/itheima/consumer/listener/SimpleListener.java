...
public class SimpleListener {
    ...
    @RabbitListener(bindings =@QueueBinding(
            exchange =@Exchange(name = "QBing.direct",type = "direct"),
            value = @Queue(name = "direct.queue6"),
            key = {"red","blue"}
    ))
    public void annotationExchange(){}

    @RabbitListener(bindings =@QueueBinding(
            exchange =@Exchange(name = "QBing.fanout",type = "fanout"),
            value = @Queue(name = "fanout.queue6")
    ))
    public void annotationFanoutExchange(){}
